...
public class Archivo {
    ...
    public static String elegirArchivoParaGuardar() {
        JFileChooser fc = new JFileChooser();
    
        // Establecer título de la ventana de guardado
        fc.setDialogTitle("Seleccionar ubicación para guardar el archivo");
    
        // Abre el cuadro de diálogo para guardar el archivo
        if (fc.showSaveDialog(new JFrame()) == JFileChooser.APPROVE_OPTION) {
            File f = fc.getSelectedFile();
    
            // Si el archivo seleccionado no tiene extensión, se puede agregar una extensión por defecto
            if (!f.getName().endsWith(".ser")) {
                f = new File(f.getAbsolutePath() + ".ser");
            }
    
            // Devuelve la ruta absoluta del archivo seleccionado
            return f.getAbsolutePath();
        } else {
            // Si el usuario cancela la selección, devuelve una cadena vacía
            return "";
        }
    }
